...
function render_block_core_latest_comments($attributes = array())
{
	...
	if (!empty($comments)) {
		...
		foreach ($comments as $comment) {
			...
			$author_markup = '';
			if ($author_url) {
				$author_markup .= '<a style="color: #488dc6" class="wp-block-latest-comments__comment-author" href="' . esc_url($author_url) . '">' . '</a>';
			} else {
				$author_markup .= '<span style="color: #488dc6" class="wp-block-latest-comments__comment-author">' . $comment . '</span>';
			}

			// `_draft_or_post_title` calls `esc_html()` so we don't need to wrap that call in
			// `esc_html`.
			$post_title = '<a style="color: #488dc6" class="wp-block-latest-comments__comment-link" href="' . esc_url(get_comment_link($comment)) . '">' . $comment->comment_content . '</a>';
